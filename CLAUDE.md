...
### Putting the Material 9 look back on the MDC components

MDC did not just restyle the components, it changed what they *are*, and the app's whole visual identity is the v9 one. What [src/styles.scss](src/styles.scss) does about that, and the two cascade rules that decide how:

- **Typography.** The M2 config now carries Roboto's own stack and MDC's letter-spacing (0.5px on body, 1.25px on buttons); v9 asked for `Roboto, "Helvetica Neue", sans-serif` with `letter-spacing: normal` at every level. Nothing in this app ever loads Roboto — no `@font-face`, no Google Fonts link — so the *second* name is the face every Material label actually renders in, and Material's current default stops at `sans-serif`, which on a Mac is the wider Helvetica. `$wingsearch-typography` restores both; the `@each` after it is only there because the config carries a bare `font-family` key alongside the per-level maps.
- **The form fields.** Every field in the app was `appearance="standard"`: a bare underline, flush left, `font-size: inherit` with a 1.125 line-height, subscript at 75% of that. MDC deleted that appearance, so they are `fill` — a *box*, inset 16px, a fixed 56px tall, hard-coded 16/24 text and 12/20 subscript. Every label therefore sat 16px right of its own underline and printed a size larger than the field's text. The block that undoes it is v9's own numbers in em of the field's font size, which is why they hold for both the 15px field in the panel header and the 14px ones in the body. Two MDC facts inside it are worth knowing before touching a select: it reads its own trigger typography tokens (16px/24px) rather than inheriting the field's, and the dropdown arrow lives in a fixed 24px box that ends up setting the field's height.
- **Material's component CSS is injected after `styles.scss`, so an equal-specificity rule of ours loses.** There are two ways through and the choice is not cosmetic. Setting **the token the framework declaration reads** always wins, because custom properties resolve per element rather than per rule — that is how the dialog's content padding and the form field's hover/focus state layer are dealt with. Raising **specificity** works where there is no token (`mat-form-field.mat-mdc-form-field` for `font-size: inherit`). What does *not* work is re-declaring the property at the same specificity, which is why `.mat-mdc-form-field-focus-overlay { background-color: transparent }` left a grey box round the language select for a build.
- **A CSS-wide keyword in a custom property is not what you meant.** `--mat-form-field-container-text-size: inherit` makes the *custom property* inherit — 16px from `html` — not `font-size: inherit`. Where the value has to be relative, the declaration has to be real.
- Two component-level v9 restorations that are not CSS: `iconPositionEnd` on the Reset button's `mat-icon`, because MDC's button template projects icons *before* the label, and `MAT_SELECT_CONFIG` with `hideSingleSelectionIndicator` in `app.module.ts`, because a single-selection option gained a trailing checkmark in v15 and the language and asset-pack lists are already keyed by flag and artwork.

Since none of this is covered by a spec — the suite does not render the shell — measure it. Every number above was checked against the live v9 site with CDP alongside `npm run preview`, not eyeballed.
...
The dialog contributes no surface of its own — the card art is the dialog — so `.card-detail-panel` is `pointer-events: none` and the card, the carousel and the close button turn it back on individually. That is what lets a click anywhere else in the panel reach the backdrop and navigate back, and it is the constraint on anything added to these dialogs: a full-width wrapper that takes pointer events would silently stop the click-outside-to-close. Below 600px the pane is widened to the viewport (`styles.scss`, `!important` because `MatDialog` writes the config's width inline) while the card keeps its own `80vw`, which is what puts the close button in a corner of the viewport rather than on the card's own corner.
