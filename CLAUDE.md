...
Two caveats. The detail dialogs still keep `cardHeight$`/`bonusCardHeight$` for the outer border radii and the carousel height — not for typography. And the old measured heights were integer `offsetHeight`, so the new fractional values differ by a fraction of a pixel (289 → 12.138px before, 289.297 → 12.1505px now); screenshot diffs against a pre-container-query build show sub-pixel antialiasing, not a regression.
