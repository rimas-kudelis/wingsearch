...
// A vendored data table, one row per base letter. Two rows are a few characters over the
// line limit and wrapping them would cost the one-row-per-letter shape that makes the table
// readable, so the rule is off for the table only.
